  ...
  return (
    <div className="text-center">
      ...
      {session.game_log.length > 0 && (
        <p className="text-sm text-gray-300 mb-4">{session.game_log[session.game_log.length - 1]}</p>
      )}
      <div className="mt-4">
        <h3 className="font-semibold mb-2 text-sm text-gray-200">Player Roles:</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {session.players.filter((p) => !p.is_spectator).map((player) => (
            <div key={player.id} className={`p-2 rounded border text-sm ${GOOD_ROLES.includes(player.role || "") ? "bg-blue-900/40 border-blue-500 text-blue-200" : "bg-red-900/40 border-red-500 text-red-200"}`}>
