...
def _build_session_state(gs: GameSession, for_player: Optional[object] = None) -> dict:
    """Build game state payload. If for_player is provided, includes personalized data."""
    active_count = len([p for p in gs.players if not p.is_spectator])
    is_game_end = gs.phase == GamePhase.GAME_END
            ...
            "players": [
                {
                    ...
                    "role": p.role if (is_game_end or (for_player and p.id == for_player.id)) else None,
                }
                for p in gs.players
            ],
            "missions": [
    ...
    if not for_player:
        state["session"]["lady_of_the_lake_holder"] = gs.lady_of_the_lake_holder

    if for_player:
        if gs.phase != GamePhase.LOBBY and for_player.role:
            state["role_info"] = get_role_info(for_player.role, gs.players)

        if for_player.id in gs.lady_of_the_lake_results:
            knowledge = []
            for target_id, allegiance in gs.lady_of_the_lake_results[for_player.id].items():
        ...
        if gs.current_mission < len(gs.missions):
            ...
            state["current_mission_details"] = {
                ...
                "votes": cm.votes,
                "mission_votes": (
                    {"player_voted": for_player.id in cm.mission_votes, "total_votes": len(cm.mission_votes)}
                    if cm.result == MissionResult.PENDING
                        ...
                        "player_voted": for_player.id in cm.mission_votes,
                    }
                ),
                "result": cm.result,
...
# Keep old names as aliases for backward compatibility with tests/imports
def _build_player_state(gs: GameSession, player) -> dict:
    return _build_session_state(gs, for_player=player)


def _build_general_state(gs: GameSession) -> dict:
    return _build_session_state(gs)
...
async def broadcast_game_state(session_id: str, db, manager: ConnectionManager):
    ...
    try:
        ...
        for player in game_session.players:
            try:
                player_state = _build_session_state(game_session, for_player=player)
        ...
        if session_id in manager.active_connections:
            ...
            general = _build_session_state(game_session)
